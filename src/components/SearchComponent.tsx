        ...
        {isLoading ? (
          ...
          searchedProducts.map((product: any, index: any) => (
            <Link to={`/products/${product.id}`} key={index} className="flex items-center justify-between my-2 border-b border-gray pb-2 w-[80%] mx-auto hover:bbg-gray">
              ...
              <p className='text-xs phone:text-sm'><span className='text-[10px] text-black italic px-2'>Price:</span>Rwf {product.price}</p>
              <p className='text-xs phone:text-sm'><span className='text-[10px] text-black italic px-2'>Category:</span>{product.category}</p>
              </div>
